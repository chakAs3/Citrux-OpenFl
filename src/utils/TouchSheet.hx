...
class TouchSheet extends Sprite
{
	public function new(contents:DisplayObject=null)
	{
		...
		addEventListener(TouchEvent.TOUCH, onTouchSheetTouch);
		useHandCursor = true;
		
		if (contents != null)
		{
			contents.x = cast(contents.width / -2);
			contents.y = cast(contents.height / -2);
			addChild(contents);
		}
	}
	...
	{
		removeEventListener(TouchEvent.TOUCH, onTouchSheetTouch);
		super.dispose();
	}
}