...
import citrux.core.CitrusObject;
import citrux.core.IState;
...
/**
	 ...
	 */
 class PoolObject extends DoublyLinkedList {

    var _poolType:Class;
  var _defaultParams:Object;
  var _poolSize:uint = 0;
  var _poolGrowthRate:uint = 0;
  var _isCitrusObjectPool:Bool;
...
// Start of the list of free objects
 var _freeListHead:DoublyLinkedListNode = null;
 var _freeCount:uint = 0;

 var gc:Vector<DoublyLinkedListNode>;
...
/**
		 ...
		 */
public function new(pooledType:Class,defaultParams:Object, poolGrowthRate:uint, isCitrusObjectPool:Bool):void {
...
/**
		 ...
		 */
  function increasePoolSize(sizeIncrease:uint,params:Object = null):void {

params = mergeParams(params);

for ( i in  0...sizeIncrease) {
...
/**
		 ...
		 * @param	params
		 */
function _create(node:DoublyLinkedListNode, params:Object = null):void {
onCreate.dispatch(cast(node.data , _poolType),params);
}
...
/**
		 ...
		 * @param	params
		 */
  function _recycle(node:DoublyLinkedListNode, params:Object = null):Void {
      onRecycle.dispatch(cast(node.data , _poolType),params)
}
...
/**
		 ...
		 * @param	params
		 */
  function _dispose(node:DoublyLinkedListNode):Void {
onDispose.dispatch(cast(node.data , _poolType));
cast(node.data , _poolType).kill = false;
}
...
/**
		 ...
		 * @param	params
		 */
  function _destroy(node:DoublyLinkedListNode):Void {
...
/**
		 ...
		 * @return Object
		 */
  function mergeParams(newParams:Object):Object
{
var p:Object = {};
var k:String;
...
public function updatePhysics(timeDelta:Float):void {

var tmpHead:DoublyLinkedListNode = head;

while (tmpHead != null) {
if(cast(tmpHead.data ,_poolType).updateCallEnabled)
cast(tmpHead.data , _poolType).update(timeDelta);

//since updatePhysics is always called, we can dispose objects set to kill here.
if ("kill" in cast(tmpHead.data , _poolType) && cast(tmpHead.data, _poolType).kill)
gc.push(tmpHead);
...
/**
		 ...
		 * @return
		 */
public function isDataDisposed(data:Dynamic):Bool
{
...
public function updateArt(stateView:ACitruxView):Void {

var tmpHead:DoublyLinkedListNode = head;

while (tmpHead != null) {
cast(tmpHead.data , _poolType).update(stateView);
tmpHead = tmpHead.next;
}
...
/** Get a node from its data
		 ...
		 * @return the node
		 */
public function getNodeFromData(data:Dynamic):DoublyLinkedListNode {
...
/**
		 ...
		 * @param	data
		 */
public function disposeFromData(data:Dynamic):DoublyLinkedListNode
{
var n:DoublyLinkedListNode = getNodeFromData(cast(data , _poolType));
if(n)
...
while (node) {
cast(node.data , CitruxObject).kill = true;
node = node.next;
}
}
...
/**
		 ...
		 */
public function foreachDisposed(callback:Function):Bool
{
...
/**
		 ...
		 * @param callback gets node.data for argument.
		 */
public function foreachRecycled(callback:Function):Bool
{
var node:DoublyLinkedListNode = head;
while (node) {
if (callback(node.data as _poolType))
...
/**
		 ...
		 * @param callback gets node.data for argument.
		 */
public function foreach(callback:Function):Bool
{
var node:DoublyLinkedListNode = head;
while (node) {
if (callback(cast(node.data , _poolType)))
return true;
node = node.next;
}
node = _freeListHead;
...
/**
		 ...
		 */
override public function get_length():uint
{
return _count;
}
...
/**
		 * returns the amount of objects currently in use.
		 */
public function get_recycledSize():uint
{
return _count;
}
...
/**
		 ...
		 */
public function get_poolSize():uint
{
return _freeCount;
}
...
/**
		 ...
		 */
public function get_allSize():uint
{
...
/**
		 ...
		 */
public function get_isCitrusObjectPool():Bool {
